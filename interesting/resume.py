...
def color(messages):
    rand_color = '\x1B[%d;%dm' % (1,random.randint(30,37))
    return '%s %s\x1B[0m' % (rand_color,messages)
...
class Resume(object):
    ...
    def show(self):
        prolist = [i for i in dir(self) if not i.startswith('') and not i.startswith('personal')]
        self.personal_information
        self.characteristics
        self.skills
        self.work_experience
        self.projectexperience
        self.findme
        for pro in prolist:
            getattr(self, pro)
...
# 直接保存文件,linux下终端执行,但是windows执行会有乱码
# 直接linux下终端执行:curl https://raw.github.com/gist/3496061/b63efdf0c1c8d0e12df4f5b905903128c951b282/My_Resume.py |python